...
export function generateMarkdownReport(report: EnhancedReport, outputDir: string): void {
    const outputPath = path.join(outputDir, 'endpointx-report.md');
...
function buildMarkdownReport(report: EnhancedReport): string {
    ...
    // Footer
    md += `---\n\n`;
    md += `*Generated by EndpointX v${metadata.packageVersion} | Node ${metadata.nodeVersion}*\n`;
